...
public abstract class AbstractHandler<R extends RequestBody, M extends MessageParameters>
	...
	private static final List<File> emptyUploadedFiles = new ArrayList<>();
				...
				.whenComplete((Void ignored, Throwable throwable) -> {
					if (throwable != null) {
						handleException(ExceptionUtils.stripCompletionException(throwable), ctx, httpRequest)
							.whenComplete((Void ignored2, Throwable throwable2) -> finalizeRequestProcessing());
					} else {
						finalizeRequestProcessing();
					}
				});
		} catch (Throwable e) {
			handleException(e, ctx, httpRequest)
				.whenComplete((Void ignored, Throwable throwable) -> finalizeRequestProcessing());
		}
	}
